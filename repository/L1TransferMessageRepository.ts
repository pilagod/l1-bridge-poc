...
export default class L1TransferMessageRepository {
  public async find(query: L1TransferMessageQuery): Promise<any | undefined> {
  ...
  public async findMany(query: L1TransferMessageQuery): Promise<any[]> {
    return new Promise((resolve, reject) => {
      const { where, params } = this.toSqlQuery(query);
      const rows: any[] = [];
      db.each(
  ...
  public async create(msg: L1TransferMessage): Promise<void> {
    return new Promise((resolve, reject) => {
      const { fields, values } = this.toSqlFieldsAndValues(msg);
  ...
  public async update(msg: L1TransferMessage): Promise<void> {
    return new Promise((resolve, reject) => {
      const { fields, values } = this.toSqlFieldsAndValues(msg);
      const sqlFields = fields.join(" = ?, ") + " = ?";
      db.run(
  ...
  public fillMessage<T extends L1DepositMessage | L1WithdrawMessage>(
  ...
  private toSqlFieldsAndValues(msg: L1DepositMessage | L1WithdrawMessage): {
    fields: string[];
    values: any[];
  } {
    return {
